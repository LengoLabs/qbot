...
exports.run = async (client, message, args) => {
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** Fired ${username} to ${fireResponse.newRole.name} (${fireResponse.newRole.rank})`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `<@${message.author.id}> has fired ${username} from ${rankNameInGroup} (${rankInGroup}) to ${fireResponse.newRole.name} (${fireResponse.newRole.rank}).`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
