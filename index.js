...
client.on("ready", () => {
  console.log(
    chalk.yellow(figlet.textSync("qbot", { horizontalLayout: "full" }))
  );
  console.log(
    chalk.yellow(
      `Bot started! This bot is currently helping ${client.users.size} users in ${client.users.size} channels of ${client.guilds.size} servers.`
    )
  );
});
...
client.on("message", async message => {
  ...
  if (command === "say") {
    if (!args[1]) {
      return message.channel.send({
        embed: {
          color: 15406156,
          description: "Please provide a message and channel.",
          author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL
          },
      ...
      message.channel.send({
        embed: {
          color: 894956,
          description: `Message sent to ${mChannel}!`,
          author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL
          },
  ...
  if (command === "setrank") {
    if (
      !message.member.roles.some(r => ["Ranking Permissions"].includes(r.name))
    )
      return message.channel.send({
        embed: {
          color: 15406156,
          description:
            "You need the `Ranking Permissions` role to run this command.",
          author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL
          }
    ...
    if (username) {
      rbx
        ...
        .then(function(id) {
          rbx
            .getRankInGroup(config.groupId, id)
            .then(function(rank) {
              if (config.maximumRank <= rank) {
                ...
                rbx
                  .setRank(config.groupId, id, rankIdentifier)
                  .then(function(newRole) {
                    ...
                    var logchannel = message.guild.channels.get(
                      config.logchannelid
                    );
                    logchannel.send({
                      embed: {
                  ...
                  .catch(function(err) {
                    ...
                    message.channel.send({
                      embed: {
                        color: 15406156,
                        description:
                          "Oops! Something went wrong. The issue has been logged to the bot console.",
                        author: {
                          name: message.author.tag,
                          icon_url: message.author.displayAvatarURL
                        }
  ...
  if (command === "shout") {
    if (
      !message.member.roles.some(r =>
        ["Ranking Permissions", "Shout Permissions"].includes(r.name)
      )
    )
      return message.channel.send({
        embed: {
          color: 15406156,
          description:
            "You need the `Ranking Permissions` or `Shout Permissions` role to run this command.",
          author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL
          }
